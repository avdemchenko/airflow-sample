...
def my_extract(**kwargs):
    ...
    payload = {'Key': 'YOUR_API_KEY', 'q': 'Berlin', 'aqi': 'no'}
...
with DAG('ETLWeatherPrintAirflow2', description='Airflow2.0 DAG', start_date=dt.datetime(2018, 11, 1)
         ,schedule_interval = "0 * * * *", catchup=False ,tags=['airflow-sample']) as dag:
    ext = PythonOperator(
        task_id='extract',
